...
char device_read_raw_sector(Blockdevice* dev, uint64_t sector, uint32_t counter, void* buffer){
    unsigned char (*foo)(Blockdevice*, upointer_t, uint32_t, void*) = (void*)dev->readcommand;
	return foo(dev,(upointer_t)(dev->offset + sector),counter,buffer);
}

char device_write_raw_sector(Blockdevice* dev, uint64_t sector, uint32_t counter, void* buffer){
    unsigned char (*foo)(Blockdevice*, upointer_t, uint32_t, void*) = (void*)dev->writecommand;
	return foo(dev,(upointer_t)(dev->offset + sector),counter,buffer);
}
