...
int find_cmdslot(HBA_PORT *port)
{
	...
	k_printf("Cannot find free command list entry\n");
	return -1;
}
...
int ahci_atapi_read(HBA_PORT *port, unsigned long startl, unsigned long starth, unsigned long count, unsigned short *buf)
{
	...
	cmdheader->ctba = (unsigned long)requestPage();
	k_printf("[AHCI] DINGES: %x \n\n",cmdheader->ctba);
...
void ahci_ata_init(HBA_PORT *port,int i){
	port_rebase(port,i);
	unsigned char* msg = (unsigned char*) requestPage();
	if(ahci_ata_read(port, 0, 0, 1, (unsigned short *)msg)==0){
		return;
	}
	if(msg[510]==0x55&&msg[511]==0xAA){
		k_printf("[AHCI] ATA is bootable\n");
	}else{
		k_printf("[AHCI] ATA is not bootable\n");
	}
}

void ahci_atapi_init(HBA_PORT *port,int i){
	port_rebase(port,i);
	unsigned char* buffer = (unsigned char*) requestPage();
	for(int z = 0 ; z < 512 ; z++){
		buffer[z] = 0x00;
	}
	k_printf("[AHCI] ATAPI read test sector...\n");
	if(ahci_atapi_read(port, 0, 0, 1, (unsigned short *)buffer)==0){
		k_printf("[AHCI] ATAPI is unable to read sectors...\n");
		k_printf("[AHCI] ATAPI will eject drive....\n");
		ahci_atapi_eject(port);
		return;
	}
	if(buffer[510]==0x55&&buffer[511]==0xAA){
		k_printf("[AHCI] ATAPI is bootable\n");
	}else{
		k_printf("[AHCI] ATAPI is not bootable\n");
	}
}

void initialise_ahci_driver(unsigned long bar5){
	//init_ide2();
	...
	while (i<33){
		if (pi & 1){
			...
			k_printf("[AHCI] Probing port %x \n",i);
		 
			if (det != HBA_PORT_DET_PRESENT && ipm != HBA_PORT_IPM_ACTIVE){
				
		 	}else if(port->sig==SATA_SIG_ATAPI){
				k_printf("[AHCI] ATAPI detected\n");
				ahci_atapi_init(port,i);
			}else if(port->sig==SATA_SIG_SEMB){
				k_printf("[AHCI] SEMB detected\n");
			}else if(port->sig==SATA_SIG_PM){
				k_printf("[AHCI] PM detected\n");
			}else{
				k_printf("[AHCI] SATA detected %x \n",i);
