...
upointer_t tftp_filesize(Filesystem* fs,char* path){
    // are we there?
    if(ethernet_is_enabled()==0){
        k_printf("tftp: ethernet is not enabled!\n");
        return 0;
...
upointer_t tftp_write(Filesystem* fs,char* path,void* bufferedcapacity,upointer_t filesize){
    return 0;
}

char tftp_read(Filesystem* fs,char* path,void *bufferedcapacity){
    upointer_t sz = tftp_filesize(NULL,path);
    void* buffer = load_tftp_file(path);
    memcpy(bufferedcapacity,buffer,sz);
    ((char*)bufferedcapacity)[sz] = 0;
    return 1;
}

char *tftp_dir(Filesystem* fs,char* path){
    void *rp = requestPage();
    tftp_read(fs,"dir.txt",rp);
    return rp;
}

void tftp_detect_and_initialise(){
    Filesystem *fs = registerFileSystem(NULL,tftp_read,tftp_dir,tftp_filesize,tftp_write);
}